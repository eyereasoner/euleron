...
impl<'a> Lexer<'a> {
    ...
    fn read_iri(&mut self) -> Result<()> {
        ...
        while let Some(ch) = self.peek() {
            ...
            if ch == '\\' {
                let Some(esc) = self.peek() else { return Err(FeyeError::at("unterminated IRI escape", offset)); };
                self.bump();
                if esc == 'u' || esc == 'U' {
                    let count = if esc == 'u' { 4 } else { 8 };
                    let mut hex = String::new();
                    for _ in 0..count {
                        let Some(h) = self.peek() else { return Err(FeyeError::at("unterminated Unicode escape", offset)); };
                        self.bump();
                        hex.push(h);
                    }
                    let code = u32::from_str_radix(&hex, 16).map_err(|_| FeyeError::at("invalid Unicode escape", offset))?;
                    let Some(c) = char::from_u32(code) else { return Err(FeyeError::at("invalid Unicode scalar value", offset)); };
                    if is_forbidden_iri_char(c) { return Err(FeyeError::at("forbidden character in IRI reference", offset)); }
                    value.push(c);
                } else {
                    return Err(FeyeError::at(format!("invalid IRI escape \\{}", esc), offset));
                }
            } else {
                if is_forbidden_iri_char(ch) {
                    return Err(FeyeError::at("forbidden character in IRI reference", offset));
                }
                value.push(ch);
            }
        }
        Err(FeyeError::at("unterminated IRI reference", offset))
    }
    ...
    fn read_string(&mut self) -> Result<()> {
        ...
        let mut value = String::new();
        loop {
            let Some(ch) = self.peek() else { return Err(FeyeError::at("unterminated string literal", offset)); };
            self.bump();
            if ch == quote {
                if triple {
                ...
                return Err(FeyeError::at("newline in short string literal", offset));
            } else if ch == '\\' {
                let Some(esc) = self.peek() else { return Err(FeyeError::at("unterminated string escape", offset)); };
                self.bump();
                match esc {
                    'n' => value.push('\n'),
                    'r' => value.push('\r'),
                    ...
                    'u' | 'U' => {
                        ...
                        let code = u32::from_str_radix(&hex, 16).map_err(|_| FeyeError::at("invalid Unicode escape", offset))?;
                        let Some(c) = char::from_u32(code) else { return Err(FeyeError::at("invalid Unicode scalar value", offset)); };
                        if is_forbidden_string_char(c) { return Err(FeyeError::at("forbidden character in string literal", offset)); }
                        value.push(c);
                    }
                    other => return Err(FeyeError::at(format!("invalid string escape \\{}", other), offset)),
                }
            } else {
                value.push(ch);
            }
    ...
    fn read_number(&mut self) -> Result<()> {
        ...
        if self.peek() == Some('.') && self.peek_next().is_some_and(|c| c.is_ascii_digit()) {
            return Err(FeyeError::at("malformed numeric literal", offset));
        }
    ...
    fn read_word(&mut self) -> Result<()> {
        ...
        if word.is_empty() {
            return Err(FeyeError::at(format!("unexpected character {:?}", self.peek()), offset));
        }
