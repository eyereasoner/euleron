...
struct Runner { cache: HashMap<String, String> }

fn main() {
    if let Err(err) = run() {
        eprintln!("w3c_rdf: {err}");
        std::process::exit(2);
    }
}
...
fn run() -> Result<(), String> {
    ...
    if opt.list {
        for url in DEFAULT_MANIFESTS { println!("{url}"); }
        return Ok(());
    }
    ...
    if counts.fail > 0 { std::process::exit(1); }
    Ok(())
}
...
fn print_help() {
    println!("Rust-only W3C RDF 1.x manifest runner for Euleron");
    println!();
    println!("Usage:");
    println!("  cargo run --bin w3c_rdf -- [options] [manifest-url-or-path ...]");
    println!();
    println!("By default, runs the same 12 W3C RDF manifests used by Eyeleng and writes reports/w3c-rdf-earl.ttl.");
    println!();
    println!("Options:");
...
impl Runner {
    fn run_manifest(&mut self, resource: &str, opt: &Options) -> Result<ManifestRun, String> {
        ...
        for (idx, case) in cases.into_iter().enumerate() {
            ...
            if !opt.quiet { eprintln!("{}", format_progress_line(&item, idx)); }
            results.push(item);
        }
    ...
    fn parse_action(&mut self, case: &ManifestCase) -> Result<Document, String> {
        ...
        let text = self.read_resource(action)?;
        let format = format_for(case, action).ok_or_else(|| format!("no parser selected for {action}"))?;
        parse_rdf12(&text, Some(action), format).map_err(|err| err.to_string())
    }
    ...
    fn parse_graph_resource(&mut self, resource: &str, type_hint: Option<&str>) -> Result<Document, String> {
        ...
        parse_rdf12(&text, Some(resource), format).map_err(|err| err.to_string())
    }
...
fn manifest_cases(doc: &Document, manifest: &str) -> Vec<ManifestCase> {
    ...
    if subjects.is_empty() {
        for t in &doc.facts {
            if matches!(&t.p, Term::Iri(p) if p == RDF_TYPE) && !subjects.contains(&t.s) {
                subjects.push(t.s.clone());
            }
        }
    }
    let mut out = Vec::new();
