use feye::{is_rdf_message_log, parse_n3, parse_n3_with_source, parse_rdf12, parse_rdf_message_log, proof_to_n3, rdf_result_to_string, reason, reason_document, result_to_string, Document, RdfFormat, ReasonerOptions};
...
fn rdf12_annotations_share_n3_lexer_parser_profile() {
    ...
    assert!(doc.facts.iter().any(|t| matches!(&t.p, feye::Term::Iri(p) if p == reifies)), "{:#?}", doc.facts);
    assert!(doc.facts.iter().any(|t| matches!(&t.p, feye::Term::Iri(p) if p == "http://example.org/source")), "{:#?}", doc.facts);
}
...
fn rdf12_parenthesized_triple_terms_remain_terms() {
    ...
    "#;
    let doc = parse_rdf12(input, Some("http://example.org/base"), RdfFormat::Turtle).unwrap();
    assert!(doc.facts.iter().any(|t| matches!(&t.o, feye::Term::Formula(inner) if inner.len() == 1)), "{:#?}", doc.facts);
}
...
fn age_example_supports_current_time_date_difference_and_duration_comparison() {
    ...
    assert!(result.derived.iter().any(|triple| {
        triple.s == feye::Term::Iri("https://example.org/#test".to_string())
            && triple.p == feye::Term::Iri("https://example.org/#is".to_string())
    }));
    let proof = proof_to_n3(&doc.prefixes, &result);
...
fn playground_html_is_packaged_for_browser_wasm() {
    ...
    assert!(html.contains("The Feye N3 Playground"), "{}", playground.display());
    assert!(html.contains("./pkg/feye.js"), "playground should load the wasm-pack web bundle");
...
fn rdf12_turtle_profile_parses_lists_through_shared_parser() {
    let doc = feye::parse_rdf12(
        r#"PREFIX : <http://example.org/>
:s :p (1 2) ."#,
        Some("http://example.org/base"),
        feye::RdfFormat::Turtle,
    ).unwrap();
    let json = feye::rdf12_json(&doc);
...
fn reasoner_reports_iteration_limit_instead_of_silent_partial_success() {
    use feye::{CompletionStatus, ReasonerLimit};
...
fn proof_output_marks_missing_support_as_unproven() {
    use feye::{CompletionStatus, ReasonerResult, ReasonerStatistics, Rule, Term, Triple};
    use feye::reasoner::DerivedFact;
