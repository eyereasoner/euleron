...
pub fn refresh_requested() -> bool { env_flag("FEYE_W3C_RDF_REFRESH") }
...
fn options_from_env(resources: Vec<String>, write_report: bool) -> Options {
    let output = std::env::var("FEYE_W3C_RDF_EARL")
        .unwrap_or_else(|_| "reports/w3c-rdf-earl.ttl".to_string());
    let filter = std::env::var("FEYE_W3C_RDF_FILTER")
        .ok()
    ...
    Options {
        ...
        filter,
        quiet: env_flag("FEYE_W3C_RDF_QUIET"),
        verbose: env_flag("FEYE_W3C_RDF_VERBOSE"),
        ..Options::default()
    }
}

fn has_filter() -> bool {
    std::env::var("FEYE_W3C_RDF_FILTER").map(|value| !value.trim().is_empty()).unwrap_or(false)
}
...
fn print_help() {
    println!("Rust-only W3C RDF 1.x manifest test runner for Feye");
    println!();
    println!("Usage:");
    ...
    println!("Network is disabled by default for speed; use FEYE_W3C_RDF_REFRESH=1 to bootstrap or refresh the local mirror.");
    println!();
    println!("Options:");
...
impl Runner {
    fn from_env() -> Self {
        let disk_cache_dir = env::var("FEYE_W3C_RDF_CACHE_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from(DEFAULT_W3C_RDF_CACHE_DIR));
        let refresh_cache = env_flag("FEYE_W3C_RDF_REFRESH");
        let online = refresh_cache || env_flag("FEYE_W3C_RDF_ONLINE");
        let offline = env_flag("FEYE_W3C_RDF_OFFLINE") || !online;
        ...
        if self.offline {
            ...
            return Err(format!(
                "local W3C RDF mirror miss for {resource}; expected {expected}. \
                 ...
                 Run once with FEYE_W3C_RDF_REFRESH=1 to populate the mirror, then commit tests/w3c_rdf/rdf-tests/."
            ));
        }
